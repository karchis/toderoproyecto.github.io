    ...
    if (empty($nombre) || empty($apellido) || empty($tipo_documento) || empty($numero_documento) || empty($fecha_nacimiento) || empty($telefono)  || empty($correo) || empty($link_whatsapp) || empty($documento_verificacion)) {
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                header("Location: mtrabajador_usuario.php?nombre=$nombre&apellido=$apellido&tipo_documento=$tipo_documento&numero_documento=$numero_documento&fecha_nacimiento=$fecha_nacimiento&telefono=$telefono&correo=$correo&link_whatsapp=$link_whatsapp&documento_verificacion=$documento_verificacion");
                exit();
            }
